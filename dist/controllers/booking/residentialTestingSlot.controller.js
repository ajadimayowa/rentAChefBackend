...
exports.getResidentialTestingSlots = void 0;
...
const generateSlotWindows = (count) => {
    const slots = [];
...
const getResidentialTestingSlots = (_req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const windows = generateSlotWindows(SLOTS_TO_GENERATE);
        const slotKeys = windows.map((window) => toDateKey(window.startDate));
        const counts = yield Booking_1.BookingModel.aggregate([
            {
                $match: {
                    workflow: enums_1.BookingWorkflow.HOME_RESIDENCE,
                    status: { $ne: enums_1.BookingStatus.CANCELLED },
        ...
        const countBySlotKey = new Map(counts.map((entry) => [entry._id, entry.count]));
...
exports.getResidentialTestingSlots = getResidentialTestingSlots;
